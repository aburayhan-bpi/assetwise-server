  ...
  try {
    ...
    app.patch("/update-name/:id", verifyToken, async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .send({ message: "Something went wrong while updating user name." });
      }
    });
      ...
      try {
        ...
        if (!userTeam) {
          ...
          return res.status(200).json({
            success: true,
            message: "New team created, employee added.",
          });
        }
        ...
        if (userTeam.members.includes(new ObjectId(empId))) {
          return res.status(400).json({
            success: false,
            message: "Employee already in your team.",
          });
        }
        ...
        return res
          .status(200)
          .json({ success: true, message: "Employee added to your team." });
      } catch (error) {
        console.error(error);
        return res
          .status(500)
    ...
    app.post("/add-selected-employees", async (req, res) => {
      ...
      try {
        ...
        if (!userTeam) {
          ...
          return res.status(200).json({
            success: true,
            message: "New team created, employees added.",
          });
        }

        // If the HR already has a team, add selected employees
        const existingMembers = userTeam.members.map((member) =>
          member.toString()
        );
        ...
        if (newMembers.length === 0) {
          return res.status(400).json({
            success: false,
            message: "All selected employees are already in your team.",
          });
        }
        ...
        await teamCollection.updateOne(
          ...
          {
            $push: {
              members: { $each: newMembers.map((id) => new ObjectId(id)) },
            },
          }
        );
        ...
        return res.status(200).json({
          success: true,
          message: "Selected employees added to your team.",
        });
      } catch (error) {
        console.error(error);
        return res
      ...
      try {
        if (email) {
          ...
          const team = employees.filter(
            (singleEmp) => singleEmp?.affiliatedWith === email
          );
        ...
        res
          .status(404)
          .send({ message: "Team not found with current hr email" });
      }
    });
    ...
    app.delete("/delete-team-member/:id", verifyToken, async (req, res) => {
      ...
      try {
        ...
        const updatedEmployee = await usersCollection.updateOne(filter, {
        ...
        if (
          updatedEmployee.modifiedCount === 1 ||
          (email && updatedTeamMember.modifiedCount === 1)
        ) {
    ...
    app.get("/my-team", verifyToken, async (req, res) => {
      ...
      try {
        const myTeam = await usersCollection
          .find({ affiliatedWith: teamEmail })
          .toArray();
    ...
    app.post("/request-asset/:email", verifyToken, async (req, res) => {
      ...
      try {
        if (requestedEmployee && requestedAssetData) {
          const result = await requestedAssetCollection.insertOne(
            requestedAssetData
          );
          // console.log(result)
    ...
    app.get("/pending-requests", async (req, res) => {
      ...
      try {
        if (empEmail) {
          ...
          res.status(400).send({
            message: "Faile to fetch my pending request data or email invalid.",
          });
        }
      } catch (err) {
        // console.log(err)
        res.status(500).send({
          message:
            "Something went wrong while fetching my pending requests asset data.",
        });
      }
    });
    ...
    app.get("/monthly-requests", async (req, res) => {
      ...
      try {
        if (empEmail) {
          ...
          res.status(400).send({
            message:
              "Failed to fetch my monthly request data. Email invalid or missing.",
          });
        }
      } catch (err) {
        console.error(err);
        res.status(500).send({
          message:
            "Something went wrong while fetching my monthly request data.",
        });
      }
    });
    ...
    app.patch("/cancel-request/:id", verifyToken, async (req, res) => {
      ...
      try {
        if (assetId) {
          const result = await requestedAssetCollection.updateOne(statusQuery, {
            $set: {
          ...
          res
            .status(404)
            .send({ message: "Failed to cancel or ID is not valid." });
        }
      } catch (err) {
        // console.log(err);
        res
          .status(500)
          .send({ message: "Something went wrong while cancel asset." });
      }
    });
    ...
    app.patch("/return-request/:id", verifyToken, async (req, res) => {
      ...
      try {
        if (assetId) {
          ...
          res
            .status(404)
            .send({ message: "Failed to return or ID is not valid." });
        }
      } catch (err) {
        // console.log(err);
        res
          .status(500)
          .send({ message: "Something went wrong while return asset." });
      }
    });
    ...
    app.patch("/update-asset-status/:id", verifyToken, async (req, res) => {
      ...
      try {
        if (id && status === "approved") {
          const result = await requestedAssetCollection.updateOne(query, {
            $set: {
              status: status,
    ...
    app.get("/top-most-requested", async (req, res) => {
      ...
      const topAssets = await requestedAssetCollection
        .aggregate([
          {
            $match: {
    ...
    app.get("/limited-stock-assets", async (req, res) => {
      ...
      if (email) {
        ...
        res
          .status(404)
          .send({ message: "Email is required or something went wrong." });
      }
    });
    ...
    app.get("/product-type-state", async (req, res) => {
      ...
      if (!email) {
        return res
          .status(404)
          .send({ message: "Not found. Email is required" });
      }
      const returnableAssets = await requestedAssetCollection
        .find(returnQuery)
        .toArray();

      const nonReturnableAssets = await requestedAssetCollection
        .find(nonReturnQuery)
        .toArray();
      ...
      const returnablePercentage = totalCount
        ? (returnableCount / totalCount) * 100
        : 0;
      const nonReturnablePercentage = totalCount
        ? (nonReturnableCount / totalCount) * 100
        : 0;
    ...
    app.post("/payment", async (req, res) => {
      ...
      try {
        ...
        if (existingPayment) {
          return res
            .status(400)
            .send({ message: "Payment data already exists!" });
        }
    ...
    app.patch("/update-payment/:id", verifyToken, async (req, res) => {
      ...
      try {
        ...
        if (existingPayment) {
          return res
            .status(400)
            .send({ message: "Payment or Package already exists!" });
        }
